 ...
 * el PIN seleccionamos, el programa se ejecute y configure todo correctamente.
 *
 */
 ...
 * Lo primero y más importante es activar la señal del reloj principal hasta ese
...
void GPIO_Config (GPIO_Handler_t *pGPIOHandler){
	...
	// Después de activado, podemos comenzaar a configurar.
	...
	// Antes de cargar el nuevo valor, limpiamos los bits específicos de ese registro (debemos escribir 0xb0)
	...
	if(pGPIOHandler->GPIO_PinConfig.GPIO_PinMode == GPIO_MODE_ALTFN){
		...
		pGPIOHandler ->pGPIOx->AFR[0] |= (pGPIOHandler->GPIO_PinConfig.GPIO_PinAltFunMode << auxPosition);
			...
			// Estamos en el resgistro AFRH, que controla los poines del PIN 8 al PIN 15
			auxPosition = 4 * (pGPIOHandler->GPIO_PinConfig.GPIO_PinNumber -8);

			// Limpiamos primero la posición del registro que deseamos escribir a continuación
			pGPIOHandler->pGPIOx->AFR[1] &= ~(0b111 << auxPosition);

			// Y escribimos eñ valor configurado en la posición seleccionada
